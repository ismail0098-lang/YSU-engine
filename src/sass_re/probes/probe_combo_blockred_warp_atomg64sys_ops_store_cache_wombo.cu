/*
 * SASS RE Probe: fused blockred/warp/64-bit sys-ATOMG + system load/store
 *
 * Goal: widen the strongest current local family by reintroducing direct
 * 64-bit system load/store helpers alongside:
 *   - async/cache backbone
 *   - BAR.RED.* + B2R.RESULT
 *   - MATCH/VOTE/REDUX
 *   - dense 64-bit ATOMG.E.*.STRONG.SYS block
 */

#include <cuda_runtime.h>
#include <stdint.h>

static __device__ __forceinline__ unsigned combo_blockred_warp_atomg64sys_store_smem_addr(unsigned char *ptr) {
...
probe_combo_blockred_warp_atomg64sys_ops_store_cache_wombo(
    ...
    const unsigned saddr =
        combo_blockred_warp_atomg64sys_store_smem_addr(smem) + ((lane & 15u) << 4);
    ...
    if ((lane & 7u) == 0u) {
        ...
        const unsigned long long observed = sys_load_src[0];
        sys_store_dst[0] = observed ^ acc ^ 0x0101010101010101ull;
