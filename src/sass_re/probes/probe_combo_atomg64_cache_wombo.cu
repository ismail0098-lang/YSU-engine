/*
 * SASS RE Probe: 64-bit ATOMG CAS + async/cache "wombo combo"
 *
 * Goal: force return-valued 64-bit atomics inside the established async/cache
 * backbone:
 *   - LDG.E.64(.STRONG.GPU)
...
static __device__ __forceinline__ unsigned combo_atomg64_smem_addr(unsigned char *ptr) {
    return static_cast<unsigned>(__cvta_generic_to_shared(ptr));
}

extern "C" __global__ void __launch_bounds__(32)
probe_combo_atomg64_cache_wombo(unsigned long long *out,
                                unsigned long long *cas64_dst,
                                const unsigned char *src_u8,
                                const unsigned long long *src_u64,
                                unsigned long long bias) {
    __shared__ __align__(16) unsigned char smem[64];
    const unsigned saddr = combo_atomg64_smem_addr(smem);
    ...
    const unsigned long long v64 =
        src_u64[lane] ^
        ((unsigned long long)(sm0 & 0xffu) << 16) ^
        ((unsigned long long)(sm1 & 0xffu) << 24) ^
        bias ^ (unsigned long long)lane;

    unsigned long long old_cas = atomicCAS(cas64_dst, v64 ^ 0x55ull, v64 ^ 0xaaull);
    out[lane] = old_cas ^ v64;
}
