/*
 * SASS RE Probe: 64-bit ATOMG EXCH + async/cache "wombo combo"
 *
 * Goal: test whether 64-bit exchange can join the same local combo family:
 *   - LDG.E.64(.STRONG.GPU)
 *   - LDGSTS.E.BYPASS.LTC128B.128(.ZFILL) + LDGDEPBAR + DEPBAR.LE
 *   - ATOMG.E.EXCH.64.? or neighboring 64-bit exchange lowering
 */

#include <cuda_runtime.h>
#include <stdint.h>

static __device__ __forceinline__ unsigned combo_atomg64_exch_smem_addr(unsigned char *ptr) {
    return static_cast<unsigned>(__cvta_generic_to_shared(ptr));
}

extern "C" __global__ void __launch_bounds__(32)
probe_combo_atomg64_exch_cache_wombo(unsigned long long *out,
                                     unsigned long long *exch64_dst,
                                     const unsigned char *src_u8,
                                     const unsigned long long *src_u64,
                                     unsigned long long bias) {
    __shared__ __align__(16) unsigned char smem[64];
    const unsigned saddr = combo_atomg64_exch_smem_addr(smem);
    ...
    unsigned long long old_exch = atomicExch(exch64_dst, v64 | 1ull);
    out[lane] = old_exch ^ v64;
}
