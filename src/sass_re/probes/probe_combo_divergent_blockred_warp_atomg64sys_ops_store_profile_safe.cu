/*
 * SASS RE Probe: runtime-safe surrogate for the divergent + SYS64 store bridge.
 *
 * Goal: preserve the already-safe divergent fused 64-bit SYS family while
 * adding direct 64-bit SYS load/store reuse, so we can profile the
 * reconvergence + store-side bridge instead of treating it as symbolic-only.
 */
...
combo_div_store_sys_smem_addr(unsigned char *ptr) {
    return static_cast<unsigned>(__cvta_generic_to_shared(ptr));
}

static __device__ __noinline__ int
combo_div_store_sys_stage(int lane, int v, int bias) {
    if ((lane & 1) == 0) {
        v = v * 3 + bias;
        if ((lane & 4) != 0) {
            v ^= 0x55;
...
probe_combo_divergent_blockred_warp_atomg64sys_ops_store_profile_safe(
    ...
    const unsigned base = combo_div_store_sys_smem_addr(smem) + lane * 16u;
    ...
    const int dv =
        combo_div_store_sys_stage((int)lane, (int)(lane + (unsigned)bias), (int)bias);
