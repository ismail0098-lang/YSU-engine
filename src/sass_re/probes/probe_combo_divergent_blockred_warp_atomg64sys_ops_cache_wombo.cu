/*
 * SASS RE Probe: divergence/reconvergence + fused blockred/warp/64-bit sys
 *
 * Goal: test whether a light reconvergence shape can add WARPSYNC-side
 * helpers to the strongest current local family without destroying:
 *   - async/cache backbone
 *   - BAR.RED.* + B2R.RESULT
 *   - MATCH/VOTE/REDUX
 *   - 64-bit ATOMG.E.*.STRONG.SYS block
 */

#include <cuda_runtime.h>
#include <stdint.h>

static __device__ __forceinline__ unsigned combo_div_blockred_warp_atomg64sys_smem_addr(unsigned char *ptr) {
    return static_cast<unsigned>(__cvta_generic_to_shared(ptr));
}

static __device__ __noinline__ int combo_div_blockred_warp_stage(int lane, int v, int bias) {
    if ((lane & 1) == 0) {
        v = v * 3 + bias;
        if ((lane & 4) != 0) {
            v ^= 0x55;
...
probe_combo_divergent_blockred_warp_atomg64sys_ops_cache_wombo(
    ...
    const unsigned saddr =
        combo_div_blockred_warp_atomg64sys_smem_addr(smem) + ((lane & 15u) << 4);

    unsigned ballot_pre = __ballot_sync(0xffffffffu, ((lane + (unsigned)bias) & 1u) != 0u);
    int dv = combo_div_blockred_warp_stage((int)lane, (int)(lane + (unsigned)bias), (int)bias);
