...
RUN_DIR="${1:-$ROOT/results/runs/combo_uniform_divergent_atomg64sys_profile_safe_tranche_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "$RUN_DIR"

NVCC_STD="$("$ROOT/scripts/resolve_nvcc_std_flag.sh")"
BIN="$RUN_DIR/combo_uniform_divergent_atomg64sys_profile_safe_runner"
SASS="$RUN_DIR/combo_uniform_divergent_atomg64sys_profile_safe_runner.sass"
LOG="$RUN_DIR/run.log"
...
nvcc -arch=sm_89 -O3 "$NVCC_STD" -Xptxas "$PTXAS_FLAGS" \
  "$ROOT/runners/combo_uniform_divergent_atomg64sys_profile_safe_runner.cu" \
  "$ROOT/probes/probe_combo_uniform_divergent_atomg64sys_profile_safe.cu" \
  -o "$BIN"

cuobjdump --dump-sass "$BIN" > "$SASS"
"$BIN" 180 | tee "$LOG"

ncu --target-processes all --kernel-name regex:probe_combo_uniform_divergent_atomg64sys_profile_safe \
...
cat > "$SUMMARY" <<EOF
combo_uniform_divergent_atomg64sys_profile_safe_tranche
======================================================

Probe:
- src/sass_re/probes/probe_combo_uniform_divergent_atomg64sys_profile_safe.cu

Runner:
- src/sass_re/runners/combo_uniform_divergent_atomg64sys_profile_safe_runner.cu
...
Goal:
- Keep the uniform helper front-end alive while adding optimized reconvergence
  pressure and the narrow 64-bit SYS atomic matrix on top of the aligned
