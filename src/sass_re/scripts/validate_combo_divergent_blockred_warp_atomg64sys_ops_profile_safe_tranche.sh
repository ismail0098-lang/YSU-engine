...
RUN_DIR="${1:-$ROOT/results/runs/combo_divergent_blockred_warp_atomg64sys_ops_profile_safe_tranche_$(date +%Y%m%d_%H%M%S)}"
mkdir -p "$RUN_DIR"

STD_FLAG="$("$ROOT/scripts/resolve_nvcc_std_flag.sh")"
BIN="$RUN_DIR/combo_divergent_blockred_warp_atomg64sys_ops_profile_safe_runner"
SASS="$RUN_DIR/combo_divergent_blockred_warp_atomg64sys_ops_profile_safe_runner.sass"
PTXAS_FLAGS="-O3"
...
/opt/cuda/bin/nvcc -arch=sm_89 ${STD_FLAG} -O3 -Xptxas "${PTXAS_FLAGS}" -lineinfo \
  "$ROOT/runners/combo_divergent_blockred_warp_atomg64sys_ops_profile_safe_runner.cu" \
  "$ROOT/probes/probe_combo_divergent_blockred_warp_atomg64sys_ops_profile_safe.cu" \
  -o "$BIN"
...
if command -v ncu >/dev/null 2>&1; then
  ncu --target-processes all --kernel-name regex:probe_combo_divergent_blockred_warp_atomg64sys_ops_profile_safe \
    --metrics \
...
combo_divergent_blockred_warp_atomg64sys_ops_profile_safe_tranche
================================================================

runner:
- src/sass_re/runners/combo_divergent_blockred_warp_atomg64sys_ops_profile_safe_runner.cu

probe:
- src/sass_re/probes/probe_combo_divergent_blockred_warp_atomg64sys_ops_profile_safe.cu

artifacts:
- $RUN_DIR/combo_divergent_blockred_warp_atomg64sys_ops_profile_safe_runner.sass
- $RUN_DIR/run.log
- $RUN_DIR/ncu.csv
- $RUN_DIR/ncu.stderr
