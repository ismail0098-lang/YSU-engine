...
RUNNER="$OUTDIR/uplop3_cubin_driver_runner"
mkdir -p "$OUTDIR"

g++ -std=c++20 -O2 "$ROOT/runners/uplop3_cubin_driver_runner.cpp" -I/opt/cuda/include -L/opt/cuda/lib64 -lcuda -ldl -o "$RUNNER"

SUMMARY="$OUTDIR/summary.txt"
{
    echo "uplop3_cubin_pattern_matrix"
    echo "==========================="
    echo
    echo "kernel=$KERNEL"
    echo
...
for pattern in 0 1 2 3; do
    ...
    for ((i = 0; i < ${#SPECS[@]}; i += 2)); do
        ...
        echo "- $label:" >> "$SUMMARY"
        echo "  rc=$rc" >> "$SUMMARY"
        if (( rc == 0 )) && [[ -n "$baseline_ref" ]] && diff -u "$baseline_log" "$log" >/dev/null; then
            echo "  relation=same_as_baseline" >> "$SUMMARY"
        elif (( rc == 0 )) && [[ -n "$baseline_ref" ]]; then
