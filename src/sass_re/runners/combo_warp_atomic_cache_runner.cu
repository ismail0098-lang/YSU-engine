...
int main(int argc, char **argv) {
    ...
    const int count_u8 = 128;
    const int count_u16 = 128;
    ...
    *d_atomic = 0;
    ...
    for (int i = 0; i < 20; ++i) {
        probe_combo_voteall_minmax_atomic_cache_wombo<<<1, threads>>>(
    ...
    printf("combo_warp_atomic_cache_ms=%.6f\n", ms);
    printf("combo_warp_atomic_cache_iters=%d\n", iters);
    printf("combo_warp_atomic_cache_checksum=%.6f\n", checksum);
    printf("combo_warp_atomic_cache_accum=%.6f\n", (double)(*d_accum));
    printf("combo_warp_atomic_cache_atomic=%d\n", *d_atomic);
