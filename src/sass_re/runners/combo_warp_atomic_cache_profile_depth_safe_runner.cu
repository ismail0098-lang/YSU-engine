...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < count_u8; ++i) h_u8[i] = (unsigned char)((i * 29 + 7) & 0xff);
    for (int i = 0; i < count_u16; ++i) h_u16[i] = (uint16_t)((i * 911 + 19) & 0xffff);
    ...
    *d_atomic = 0;
    ...
    for (int i = 0; i < 20; ++i) {
        probe_combo_warp_atomic_cache_profile_depth_safe<<<1, threads>>>(
    ...
    printf("combo_warp_atomic_cache_profile_depth_safe_ms=%.6f\n", ms);
    printf("combo_warp_atomic_cache_profile_depth_safe_iters=%d\n", iters);
    printf("combo_warp_atomic_cache_profile_depth_safe_checksum=%.6f\n", checksum);
    printf("combo_warp_atomic_cache_profile_depth_safe_accum=%.6f\n", (double)(*d_accum));
    printf("combo_warp_atomic_cache_profile_depth_safe_atomic=%d\n", *d_atomic);
