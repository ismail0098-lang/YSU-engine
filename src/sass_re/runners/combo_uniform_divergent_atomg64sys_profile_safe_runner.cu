...
int main(int argc, char **argv) {
    const int iters = (argc > 1) ? atoi(argv[1]) : 180;
    ...
    for (int i = 0; i < count_u8; ++i) {
        h_u8[i] = (unsigned char)((i * 31 + 9) & 0xff);
    }
    for (int i = 0; i < count_u64; ++i) {
        h_u64[i] = 0x2468ace000000000ull ^
    ...
    *d_xor = 0ull;
    ...
    for (int i = 0; i < 16; ++i) {
        probe_combo_uniform_divergent_atomg64sys_profile_safe<<<1, threads>>>(
    ...
    for (int i = 0; i < iters; ++i) {
        probe_combo_uniform_divergent_atomg64sys_profile_safe<<<1, threads>>>(
            d_out, d_add, d_min, d_max, d_and, d_or, d_xor,
            d_u8, d_u64, 0x1029384756abcde0ull + (unsigned long long)i,
            3 + (i & 1), 5, 127);
    }
    ...
    printf("combo_uniform_divergent_atomg64sys_profile_safe_ms=%.6f\n", ms);
    printf("combo_uniform_divergent_atomg64sys_profile_safe_iters=%d\n", iters);
    printf("combo_uniform_divergent_atomg64sys_profile_safe_checksum=0x%016llx\n",
