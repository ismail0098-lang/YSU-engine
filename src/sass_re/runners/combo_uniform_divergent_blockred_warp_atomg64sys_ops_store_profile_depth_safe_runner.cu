...
int main(int argc, char **argv) {
    ...
    *d_xor = 0ull;
    ...
    for (int i = 0; i < 16; ++i) {
        probe_combo_uniform_divergent_blockred_warp_atomg64sys_ops_store_profile_depth_safe<<<1, threads>>>(
            d_out, d_add, d_min, d_max, d_and, d_or, d_xor,
            d_sys_store, d_sys_load, d_u8, d_u64,
            0x1029384756abcde0ull, 3, 5, 127);
    }
    ...
    for (int i = 0; i < iters; ++i) {
        probe_combo_uniform_divergent_blockred_warp_atomg64sys_ops_store_profile_depth_safe<<<1, threads>>>(
            d_out, d_add, d_min, d_max, d_and, d_or, d_xor,
            d_sys_store, d_sys_load, d_u8, d_u64,
            0x1029384756abcde0ull + (unsigned long long)i, 3 + (i & 1), 5, 127);
    }
    ...
    printf("combo_uniform_divergent_blockred_warp_atomg64sys_ops_store_profile_depth_safe_ms=%.6f\n", ms);
    printf("combo_uniform_divergent_blockred_warp_atomg64sys_ops_store_profile_depth_safe_iters=%d\n", iters);
    printf("combo_uniform_divergent_blockred_warp_atomg64sys_ops_store_profile_depth_safe_checksum=0x%016llx\n",
