...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < count_u8; ++i) {
        h_u8[i] = (unsigned char)((i * 29 + 11) & 0xff);
    }
    for (int i = 0; i < count_u64; ++i) {
        h_u64[i] = 0x89abcdef00000000ull ^
    ...
    *d_xor = 0ull;
    ...
    for (int i = 0; i < 16; ++i) {
        probe_combo_blockred_warp_atomg64sys_ops_profile_depth_safe<<<1, threads>>>(
            d_out, d_add, d_min, d_max, d_and, d_or, d_xor,
            d_u8, d_u64, 0x2468ace013579bdfull);
    }
    ...
    for (int i = 0; i < iters; ++i) {
        probe_combo_blockred_warp_atomg64sys_ops_profile_depth_safe<<<1, threads>>>(
            d_out, d_add, d_min, d_max, d_and, d_or, d_xor,
            d_u8, d_u64, 0x2468ace013579bdfull + (unsigned long long)i);
    }
    ...
    printf("combo_blockred_warp_atomg64sys_ops_profile_depth_safe_ms=%.6f\n", ms);
    printf("combo_blockred_warp_atomg64sys_ops_profile_depth_safe_iters=%d\n", iters);
    printf("combo_blockred_warp_atomg64sys_ops_profile_depth_safe_checksum=0x%016llx\n",
