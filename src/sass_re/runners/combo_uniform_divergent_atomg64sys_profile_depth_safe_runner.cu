...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < count_u8; ++i) {
        h_u8[i] = (unsigned char)((i * 31 + 11) & 0xff);
    }
    ...
    *d_xor = 0ull;
    ...
    for (int i = 0; i < 16; ++i) {
        probe_combo_uniform_divergent_atomg64sys_profile_depth_safe<<<1, threads>>>(
    ...
    printf("combo_uniform_divergent_atomg64sys_profile_depth_safe_ms=%.6f\n", ms);
    printf("combo_uniform_divergent_atomg64sys_profile_depth_safe_iters=%d\n", iters);
    printf("combo_uniform_divergent_atomg64sys_profile_depth_safe_checksum=0x%016llx\n",
